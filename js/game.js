...
(() => {
  ...
  function loadOpt(){ try{ const r=JSON.parse(localStorage.getItem('thronerush_opt')); if(r&&typeof r==='object') return {shake:r.shake==null?1:r.shake,fx:r.fx==null?1:r.fx,curses:r.curses==null?true:r.curses,guns:r.guns==null?true:r.guns,dmg:r.dmg==null?true:r.dmg,dailyShop:r.dailyShop==null?true:r.dailyShop,telemetry:r.telemetry==null?false:!!r.telemetry,tlmAsked:r.tlmAsked==null?false:!!r.tlmAsked,dbg:false,
    music:r.music==null?(r.muted?0:1):volNum(r.music), sfx:r.sfx==null?(r.muted?0:1):volNum(r.sfx), fullscreen:r.fullscreen==null?true:r.fullscreen}; }catch(e){} return {shake:1,fx:1,curses:true,guns:true,dmg:true,dailyShop:true,telemetry:false,tlmAsked:false,dbg:false,music:1,sfx:1,fullscreen:true}; }
  ...
  const GAME_VER='v329';   // mit der service-worker-CACHE-Version synchron halten (taucht in der Telemetrie als `ver` auf)
