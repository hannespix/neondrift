...
(() => {
  ...
  function loadOpt(){ try{ const r=JSON.parse(localStorage.getItem('thronerush_opt')); if(r&&typeof r==='object') return {shake:r.shake==null?1:r.shake,fx:r.fx==null?1:r.fx,madnessFx:r.madnessFx==null?true:!!r.madnessFx,curses:r.curses==null?true:r.curses,guns:r.guns==null?true:r.guns,dmg:r.dmg==null?true:r.dmg,dailyShop:r.dailyShop==null?true:r.dailyShop,telemetry:r.telemetry==null?false:!!r.telemetry,tlmAsked:r.tlmAsked==null?false:!!r.tlmAsked,dbg:false,
    music:r.music==null?(r.muted?0:1):volNum(r.music), sfx:r.sfx==null?(r.muted?0:1):volNum(r.sfx), fullscreen:r.fullscreen==null?true:r.fullscreen}; }catch(e){} return {shake:1,fx:1,madnessFx:true,curses:true,guns:true,dmg:true,dailyShop:true,telemetry:false,tlmAsked:false,dbg:false,music:1,sfx:1,fullscreen:true}; }
  ...
  function reset(){
    ...
    runOrbs=0; runPerfect=0; runBosses=0; madnessTime=0; runMaxMult=1; runSPgain=0; runHits=0; onbDrops=0; runChipsPaid=0; runChipsEarned=0; coinSaveAcc=0; runJumps=0; runLoot=0; inputDirty=false; idleStretch=0; runIdleMax=0; runIdleAcc=0; breatherT=rand(24,32); breatherActive=0; mkCount=0; mkTimer=0; coachQueue=[]; coachCd=0; coachCard=null; coachPause=false; pickMissions();
  ...
  function collectLoot(L){ const it=L.it; let off=false;
  ...
  const continueCost=()=> Math.round((150+(level||1)*45)*Math.pow(2,runContinues)/10)*10;   // jedes Weiterleben doppelt so teuer wie das davor
  function continueEligible(){ return mode!=='zen' && (meta.chips||0)>=continueCost(); }   // unbegrenzt – solange du es dir leisten kannst
  ...
  function draw(){
    ...
    if(state===S.PLAY||state===S.OVER||state===S.UPGRADE||state===S.PAUSE){
      ...
      if(loot) for(const L of loot){ const it=L.it, pr=L.r+Math.sin(L.pulse)*2.2, col=it.col, gr=pr*2.6; ctx.save(); ctx.translate(L.x,L.y);
        ...
        ctx.beginPath(); for(let k=0;k<6;k++){ const a=k/6*6.283; const px=Math.cos(a)*pr,py=Math.sin(a)*pr; if(k) ctx.lineTo(px,py); else ctx.moveTo(px,py); } ctx.closePath(); ctx.fill(); ctx.stroke();
        ctx.rotate(-L.rot); ctx.fillStyle='#fff'; ctx.font='900 13px Orbitron, sans-serif'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('✦',0,1); ctx.restore(); }
  ...
  const GAME_VER='v342';   // mit der service-worker-CACHE-Version synchron halten (taucht in der Telemetrie als `ver` auf)
