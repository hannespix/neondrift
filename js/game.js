...
(() => {
  ...
  function submitScore(cb){ try{
      const r=lastRunLB; if(!r||!r.eligible){ cb&&cb(false,'inelig'); return; }
      const nick=getNick(); if(!nick){ cb&&cb(false,'nonick'); return; }
      if(!TELEMETRY_URL){ cb&&cb(false,'nourl'); return; }
      const body=JSON.stringify({nick, cid:telemetryCid(), ver:GAME_VER, score:r.score, mode:r.mode, daily:r.daily, dailyDate:r.dailyDate, lvl:r.lvl, boss:r.boss});
  ...
  function openLeaderboard(from){ lbReturn=from||'start';
    ...
    renderLbTabs(); loadLbBoard(); try{ beep(660,0.06,'square',0.2);}catch(e){} }
  ...
  function renderLbTabs(){ const host=document.getElementById('lbTabs'); if(!host) return;
    ...
    host.innerHTML=''; for(const [key,label] of tabs){ const b=document.createElement('button');
      ...
      b.addEventListener('click',()=>{ if(lbTab===key) return; lbTab=key; renderLbTabs(); loadLbBoard(); });
      host.appendChild(b); } }
  ...
  function applyI18n(){ try{ document.documentElement.lang=lang;
    ...
    set('lbBtn',t('lbBtn')); set('saveScoreBtn',t('lbSave')); set('lbBackBtn',t('lbBack')); set('lbTitleEl',t('lbTitle'));
