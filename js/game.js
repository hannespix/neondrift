...
(() => {
  ...
  const TR={
    ...
    en:{
      ...
      setTitle:'SETTINGS', tapToggle:'Tap to toggle', optMusic:'🎵 Music', optSfx:'🔊 Game Sound', optFull:'⛶ Fullscreen', coins:'Coins', coinBalLbl:'Balance', coinSoon:'Purchases coming soon', devHint:'Redeem dev/tester code', redeem:'REDEEM', devBad:'Invalid code', devPlace:'e.g. dev1000', optShake:'📳 Screenshake & rumble', optFx:'Screen effects', optCurses:'🎲 Funny curses', optGuns:'🔫 Shooting / weapons', optDmg:'🔢 Damage numbers', optTelemetry:'📊 Anonymous stats', optFps:'📈 FPS counter', tlmExport:'📤 Export telemetry', tlmDone:'✓ Copied', tlmEmpty:'no data yet', optLang:'🌐 Language',
  ...
  function draw(){
    ...
    if(opt.dbg && (state===S.PLAY||state===S.PAUSE)){ ctx.save(); ctx.fillStyle='rgba(0,0,0,0.55)'; ctx.fillRect(6,52,182,58);
      ...
      ctx.restore(); }
    if(opt.fps){ const fps=Math.round(1000/Math.max(1,frameMs));   // Perf-Anzeige: FPS · Framezeit · erkannte Rate · FX-Qualität · Render-Skala
      ctx.save(); ctx.textAlign='center'; ctx.textBaseline='top'; ctx.font='700 14px Space Mono, monospace';
      const txt=fps+' FPS · '+frameMs.toFixed(1)+'ms · '+Math.round(1000/vsyncMs)+'Hz · fxQ'+fxQ.toFixed(2)+' · q'+qScale.toFixed(2);
      const tw=ctx.measureText(txt).width+16, bx=W/2-tw/2, by=H-30;   // unten-mittig (frei von Punch-Hole/HUD)
      ctx.fillStyle='rgba(0,0,0,0.55)'; ctx.fillRect(bx,by,tw,20);
      ctx.fillStyle=fps>=80?'#7cff2e':(fps>=50?'#ffe600':'#ff6a6a'); ctx.fillText(txt,W/2,by+3); ctx.restore(); }
    ctx.restore();
  }
  ...
  async function renderCoinShop(){ updateAllBalances();
    ...
    if(!ok){ if(soon){ soon.textContent=t('coinSoon'); soon.style.display=''; } packs.forEach(b=>{ b.disabled=true; }); return; }
  ...
  const OPTLBL={music:'optMusic',sfx:'optSfx',fullscreen:'optFull',shake:'optShake',fx:'optFx',curses:'optCurses',guns:'optGuns',dmg:'optDmg',telemetry:'optTelemetry',fps:'optFps',lang:'optLang'};
