...
(() => {
  ...
  function grantWeaponDrop(){   // Waffen-Kiste: neue Waffe (run-lokal, ohne Bauplan/SP) oder +1 Skillpunkt
    const free=arsenal.slots-ownedCount(), missing=WEAPONS.map(w=>w.id).filter(id=>!arsenal.w[id]);
    if(free>0 && missing.length){ const id=pick(missing); arsenal.w[id]={lvl:1,f1:null,f2:null,f3:null,f4:null,spent:0}; recalcArsenal(); return {kind:'new',id}; }
  ...
  function collectLoot(L){ const it=L.it;
    if(it.wep){ const r=grantWeaponDrop();   // Waffen-Kiste
      spawnParticles(L.x,L.y,it.col,18,300); flash=Math.min(0.55,flash+0.16); flashColor=it.col; sfxUpgrade(); vibe([16,22,16]); runLoot++;
      if(r.kind==='new'){ banner={text:'✦ '+t('lootWeapon').toUpperCase(),sub:WID[r.id].ico+' '+wName(r.id),t:2.4,color:it.col}; floatText(L.x,L.y-20,WID[r.id].ico,it.col,28); }
  ...
  function draw(){
    ...
    if(state===S.PLAY||state===S.OVER||state===S.UPGRADE||state===S.PAUSE){
      ...
      if(loot) for(const L of loot){ const it=L.it, pr=L.r+Math.sin(L.pulse)*2.2, col=it.col, gr=pr*2.6; ctx.save(); ctx.translate(L.x,L.y);
        ...
        else { ctx.beginPath(); for(let k=0;k<6;k++){ const a=k/6*6.283; const px=Math.cos(a)*pr,py=Math.sin(a)*pr; if(k) ctx.lineTo(px,py); else ctx.moveTo(px,py); } ctx.closePath(); ctx.fill(); ctx.stroke(); }
  ...
  const GAME_VER='v344';   // mit der service-worker-CACHE-Version synchron halten (taucht in der Telemetrie als `ver` auf)
