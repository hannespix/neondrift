...
(() => {
  ...
  function gameOver(){ state=S.OVER; clearRun(); sfxGameOver(); duckMusic(2.4); bigDeathBlast(player.x,player.y);
    ...
    if(rs){ let h=t('reached')+' '+t('level')+' '+(level||1)+'  ·  '+(runBosses||0)+' '+t('bosses')+'  ·  '+t('rankLbl')+' '+(P('ranks')[rankIdx]||''); if(spChips) h+='  ·  💠'+spLeft+' → 🪙+'+spChips; rs.textContent=h; }
