...
(() => {
  ...
  const META=[
    ...
    {id:'veteran',     ico:'🎖️',name:'Veteran',      base:300,max:3},
    ...
    {id:'shield',      ico:'🛡️',name:'Startschild',  base:90, max:3},
    {id:'tough',       ico:'💗',name:'Zähigkeit',    base:300,max:2},
  ...
  function lore(id){ const e=LORE[id]; if(!e||(meta.seen&&meta.seen['lore_'+id])) return; meta.seen=meta.seen||{}; meta.seen['lore_'+id]=1; saveMeta();
    const tx=(e[lang]||e.en); banner={text:'📖 '+tx[0],sub:tx[1],t:4.4,color:'#c45bff'}; try{ beep(523,0.08,'sine',0.2); setTimeout(()=>beep(392,0.1,'sine',0.18),110); }catch(_){} }
  ...
  function reset(){
    ...
    banner=null; effects={slowmo:0,magnet:0,double:0,mirror:0}; mirrorOn=false; activeCurses=[]; shields=0; invuln=0; upgradeCounts={}; lives=3; runContinued=false; pendingContinue=false; jumpCharge=1; jumping=0;
  ...
  function openUpgrade(armed){ accrueChips(); updateRunShopBtns(); lore('upgrade');   // Live-Chips gutschreiben; Skillpunkte sind persistent (Boss-/Zufalls-Drops, kaufbar) → im Hangar einsetzbar
  ...
  function tryJump(){
    ...
    jumpCharge=0; jumping=JUMP_DUR; invuln=Math.max(invuln,JUMP_DUR+0.2);
  ...
  function update(dt){
    ...
    if(jumping>0) jumping-=dt; else if(jumpCharge<1) jumpCharge=Math.min(1,jumpCharge+dt/JUMP_CD);
    ...
    if(mods.slip){   // Bananen-Boden: trägheits-/impulsbasiert → das Schiff driftet, übersteuert und rutscht (deutlich schwerer zu steuern)
      ...
      const fol=mods.follow*(jumping>0?2.7:1);   // beim Sprung schneller nachziehen → echter Satz weg von der Gefahr
  ...
  function gameOver(){ state=S.OVER; clearRun(); sfxGameOver(); duckMusic(2.4); bigDeathBlast(player.x,player.y);
    ...
    if(rs){ let h=t('reached')+' '+t('level')+' '+(level||1)+'  ·  '+(runBosses||0)+' '+t('bosses'); if(spChips) h+='  ·  💠'+spLeft+' → 🪙+'+spChips; rs.textContent=h; }
  ...
  function renderAch(){ const got=ACH.filter(a=>meta.ach&&meta.ach[a.id]).length, sub=document.getElementById('achSub');
    ...
    const lw=document.getElementById('loreCards'); if(lw){ lw.innerHTML='';
      ids.forEach(id=>{ const seen=!!(meta.seen&&meta.seen['lore_'+id]), tx=(LORE[id][lang]||LORE[id].en); const c=document.createElement('div'); c.className='lcard'+(seen?'':' lock');
        c.innerHTML=seen?('<h5>📖 '+tx[0]+'</h5><p>'+tx[1]+'</p>'):('<h5>🔒 ???</h5><p>'+t('loreLocked')+'</p>'); lw.appendChild(c); }); } }
