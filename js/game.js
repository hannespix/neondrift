...
(() => {
  ...
  function reset(){
    ...
    runOrbs=0; runPerfect=0; runBosses=0; madnessTime=0; runMaxMult=1; runSPgain=0; runHits=0; onbDrops=0; runChipsPaid=0; runChipsEarned=0; coinSaveAcc=0; runJumps=0; runOnBeat=0; runLetters=0; breatherT=rand(24,32); breatherActive=0; hype=0; hypeT=0; pickMissions();
  ...
  function update(dt){
    ...
    if(!bossActive && breatherActive<=0){ spawnT-=dt; if(spawnT<=0) spawnQueued=true;
      if(spawnQueued && onStep){ spawnObstacle(); spawnQueued=false;
        spawnT=Math.max(0.28,(1.15-difficulty*0.05-level*0.022)*(mods.spawnMult||1)*(1-(director-0.5)*0.28)*difDen()*(1+1.9*introT())/diffDen); } }   // Feel-Good: etwas dünner (Basis 1.15 + Floor 0.28)
